...
首先，将每个波长下实验测得的三原色所需的量标准化使得其和为1。通过这样的操作，我们可以知道三原色按何种比例分布就得到了与该光谱色一致的色度，而调整它们总量的大小就能改变它们的明度。从此，我们就分离了色度与明度两个量。标准化后的三个数值就被称为三原色坐标，与后来的色度坐标的概念是一致的，因为色度就是颜色三要素中除去明度之外饱和度与色相的总称。
